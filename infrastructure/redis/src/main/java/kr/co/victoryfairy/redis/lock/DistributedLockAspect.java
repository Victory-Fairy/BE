 ...
 * <p>
 * {@code @Order(Ordered.HIGHEST_PRECEDENCE)}로 설정하여 트랜잭션보다 먼저 실행됩니다. 이를 통해 락 획득 → 트랜잭션 시작
 * → 비즈니스 로직 → 트랜잭션 커밋 → 락 해제 순서가 보장됩니다.
 */
@Slf4j
@Aspect
@Component
...
public class DistributedLockAspect {
    ...
    public Object distributedLock(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (dynamicValue.isEmpty()) {
            throw new IllegalStateException(String.format("분산락 키가 비어있습니다 - method: %s, key expression: %s",
                    method.getName(), annotation.key()));
        }
        ...
        try {
            ...
            if (!isLocked) {
                ...
                throw new IllegalStateException(String.format("분산락 획득 실패 - method: %s, key: %s", methodName, lockKey));
            }
